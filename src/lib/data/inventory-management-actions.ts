...
export async function loadManagementOptions(): Promise<{ catalog: CatalogEntry[]; statuses: string[] }> {
  ...
  return { catalog: catalog.map(row => ({ deviceType: row.device_type, manufacturer: row.manufacturer, model: row.model })), statuses };
}
