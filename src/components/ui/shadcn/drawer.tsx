 ...
 * and the focus and scroll handling. What is changed is every colour and
 * radius — the scrim is ours (a real blur rather than black at 50%), the panel
 * sits on the overlay step of the surface ladder, and the corner radius is the
 * sheet radius from `tokens.css`. No hex is typed here.
 */
...
function DrawerOverlay({
  ...
  return (
    <DrawerPrimitive.Overlay
      ...
      className={cn(
        "drawer-scrim fixed inset-0 z-50 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:animate-in data-[state=open]:fade-in-0",
        className
      )}
      {...props}
    />
  ...
  return (
    <DrawerPortal data-slot="drawer-portal">
      ...
      <DrawerPrimitive.Content
        ...
        className={cn(
          "group/drawer-content fixed z-50 flex h-auto flex-col bg-popover text-foreground",
        ...
        {...props}
      >
        <div className="mx-auto mt-3 hidden h-1 w-10 shrink-0 rounded-full bg-muted-foreground/40 group-data-[vaul-drawer-direction=bottom]/drawer-content:block" />
        {children}
      </DrawerPrimitive.Content>
    </DrawerPortal>
  )
