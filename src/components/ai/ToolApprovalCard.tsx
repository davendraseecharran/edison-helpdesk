 ...
 * the real call, not a paraphrase. A contained pulse from `border-beam`
 * marks it as the thing that is waiting, per the plan's rules for that
 * library.
 */
...
/*
 * The library's `mono` pulse is tuned for a card on a dark page and barely
 * registers on a panel surface, so its own custom properties lift the ring
 * and the inner glow; `strength` stays the 0.5 the plan asks for. Inline,
 * because the library injects its rules after the stylesheet.
 */
const BEAM_STYLE = {
