...
- **Border beam:** `border-beam` wraps (1) the AI panel while a request is in flight (`size="line"`, `colorVariant="mono"`, `strength` 0.35, `theme` from resolved theme, `active` only while streaming), (2) tool approval cards awaiting a decision (`size="pulse-inner"`, strength 0.5), and (3) the command palette frame on open for one cycle. Nowhere else — it is a signal that something is live, not decoration.
