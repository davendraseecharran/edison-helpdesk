...
## Addendum 2: libraries from libraries.dev (user request)

Verified on npm (MIT, React ≥18): `thinking-orbs@0.3.1`, `border-beam@1.3.0`, `liquid-gooey@0.2.1`; also `cmdk@1.1.1`, `motion@13.2.0`, `qrcode@1.5.4`, `lucide-react` (latest). Pin exact versions.

- **Task 27 (amended again):** use `thinking-orbs` (`import { ThinkingOrb } from 'thinking-orbs'`, props `state`, `size` 64 | 20, `speed`, `dark`, `paused`) instead of a hand-built orb. State mapping in `src/components/ai/Orb.tsx`: idle → `breathing`; thinking (model streaming reasoning) → `solving`; streaming text → `composing`; tool running → `working` (search tools → `searching`); listening (mic) → `listening`; connecting (device auth polling) → `connecting`; error → `paused` with a `--bad` tint overlay. `dark` follows `useTheme().resolved`. Keep `scripts/orb-preview.cjs` and the `/dev/orb` route to screenshot every state in both themes; if a state looks weak, tune `speed` or layer a subtle brass glow behind it rather than replacing the library. The `AiToggle` uses size 20 while work is in flight.
- **Border beam:** `border-beam` in `colorVariant="mono"`, `size="md"` (large, visible), fast (`strength` 0.6, quickest speed the library allows) and shown for one or two rotations only, on two moments: the AI panel opening and the command palette opening. Implement as `OpenBeam({ children, cycles = 2 })` in `src/components/ui/OpenBeam.tsx`: `active` becomes true on mount and flips false after `cycles` rotations (measure one rotation from the library's animation duration; fall back to 1.4s per cycle), then the wrapper renders children only. Tool approval cards use `size="pulse-inner"`, strength 0.5, while awaiting a decision. Nowhere else, and never while the model is merely streaming — that is the orb's job.
- **Gooey:** `liquid-gooey` powers the phone floating action cluster in `BottomTabs`: the center Lookup tab expands into three gooey items (Scan with phone, New ticket, Ask the assistant) that merge on open/close (`blur={6} contrast={18}`, `fill` = `--surface`), with a plain fallback (three ordinary buttons) under reduced motion. Nowhere else.
- **cmdk** for the palette (Task 19 amendment). **motion** for Task 32. **qrcode** server-side in Task 31.
