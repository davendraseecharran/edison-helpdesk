...
`20260914010000_staff_directory_options.sql`. This branch adds forty-two
...
(`requesters.archived_at`, `if not exists`). Every one of the forty-two also sorts
...
```bash
npx supabase migration list --linked   # nothing local pending, 42 remote-missing
```
...
```bash
npx supabase db push        # applies the 42 pending migrations, in version order
...
**The rehearsal.** Applying these forty-two migrations onto a database
...
cp -a <branch>/supabase/migrations supabase/migrations         # 61 files
npx supabase migration up --local --include-all
# -> Applying migration 20260914100000_m5_foundation.sql
# -> ... 42 files ...
# -> Applying migration 20260916110000_m5_sign_in_methods.sql
# -> {"applied":[ ...42 paths... ],"message":"Migrations applied"}
npx supabase migration list --local        # 61 rows, local == remote on every one
...
sixty-one rather than from the incremental path; the database suite is the one
