...
`20260914010000_staff_directory_options.sql`. This branch adds forty-seven
...
(`requesters.archived_at`, `if not exists`). Every one of the forty-seven also sorts
...
```bash
npx supabase migration list --linked   # nothing local pending, 47 remote-missing
```
...
```bash
npx supabase db push        # applies the 47 pending migrations, in version order
...
**The rehearsal.** Applying these forty-seven migrations onto a database
...
cp -a <branch>/supabase/migrations supabase/migrations         # 66 files
npx supabase migration up --local --include-all
# -> Applying migration 20260914100000_m5_foundation.sql
# -> ... 47 files ...
# -> Applying migration 20260916130300_m5_group_events.sql
# -> {"applied":[ ...47 paths... ],"message":"Migrations applied"}
npx supabase migration list --local        # 66 rows, local == remote on every one
...
sixty-six rather than from the incremental path; the database suite is the one
