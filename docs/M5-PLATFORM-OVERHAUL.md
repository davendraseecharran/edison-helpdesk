...
`20260914010000_staff_directory_options.sql`. This branch adds forty-three
...
(`requesters.archived_at`, `if not exists`). Every one of the forty-three also sorts
...
```bash
npx supabase migration list --linked   # nothing local pending, 43 remote-missing
```
...
```bash
npx supabase db push        # applies the 43 pending migrations, in version order
...
**The rehearsal.** Applying these forty-three migrations onto a database
...
cp -a <branch>/supabase/migrations supabase/migrations         # 62 files
npx supabase migration up --local --include-all
# -> Applying migration 20260914100000_m5_foundation.sql
# -> ... 43 files ...
# -> Applying migration 20260916120000_m5_password_minimum.sql
# -> {"applied":[ ...43 paths... ],"message":"Migrations applied"}
npx supabase migration list --local        # 62 rows, local == remote on every one

# 3. the suites, against that database
npx vitest run --config vitest.db.config.mts    # 492 tests, 37 files, all passing
npx vitest run --config vitest.auth.config.mts  # 57 tests, 8 files, all passing
```
...
sixty-two rather than from the incremental path; the database suite is the one
