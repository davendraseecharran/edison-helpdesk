...
`20260914010000_staff_directory_options.sql`. This branch adds forty-eight
...
(`requesters.archived_at`, `if not exists`). Every one of the forty-eight also sorts
...
```bash
npx supabase migration list --linked   # nothing local pending, 48 remote-missing
```
...
```bash
npx supabase db push        # applies the 48 pending migrations, in version order
...
**The rehearsal.** Applying these forty-eight migrations onto a database
...
cp -a <branch>/supabase/migrations supabase/migrations         # 67 files
npx supabase migration up --local --include-all
# -> Applying migration 20260914100000_m5_foundation.sql
# -> ... 48 files ...
# -> Applying migration 20260916140000_m5_gmail_direct.sql
# -> {"applied":[ ...48 paths... ],"message":"Migrations applied"}
npx supabase migration list --local        # 67 rows, local == remote on every one
...
sixty-seven rather than from the incremental path; the database suite is the one
