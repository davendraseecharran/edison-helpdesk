...
`20260914010000_staff_directory_options.sql`. This branch adds forty-one
...
(`requesters.archived_at`, `if not exists`). Every one of the forty-one also sorts
...
```bash
npx supabase migration list --linked   # nothing local pending, 41 remote-missing
```
...
```bash
npx supabase db push        # applies the 41 pending migrations, in version order
...
**The rehearsal.** Applying these forty-one migrations onto a database
...
cp -a <branch>/supabase/migrations supabase/migrations         # 60 files
npx supabase migration up --local --include-all
# -> Applying migration 20260914100000_m5_foundation.sql
# -> ... 41 files ...
# -> Applying migration 20260916100200_m5_import_resolved.sql
# -> {"applied":[ ...41 paths... ],"message":"Migrations applied"}
...
sixty rather than from the incremental path; the database suite is the one
