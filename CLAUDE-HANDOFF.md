...
- The local stack runs on 55321/55322/55323 through an **uncommitted** patch to `supabase/config.toml`; a second worktree runs its own stack on 56321/2/3 with a distinct `project_id`. Never stage that file. Never `git add -A`; add files by name. Never commit `.env.local`. The DB and auth suites derive their container name from the worktree's own `project_id`, so a run in one worktree never resets another's database.
...
`scripts/review-overhaul.cjs` creates its own synthetic administrator and technician through the local admin API, seeds directory records, machines and tickets through the ordinary RPCs, signs in with the password form, and walks every route at 1440×900, 1024×768 and 390×844 on both themes, asserting no horizontal overflow and no browser console errors. PNGs land in `/tmp/edison-overhaul-review/final` unless `REVIEW_OUTPUT_DIR` says otherwise.
