 ...
 * fill-rule evenodd). Copied rather than imported — this script runs in a
 * browser page with no bundler and that package ships JSX — and the package is
 * not a dependency of the application. The same path is inline in
 * `src/components/ai/AiMark.tsx`; the two must stay the same artwork.
 */
const OPENAI_PATH =
