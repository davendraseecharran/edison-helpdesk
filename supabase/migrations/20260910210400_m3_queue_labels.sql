-- Review fix: account RLS hides other account rows even on visible tickets.
-- Use the authorized label-only directory, keeping ticket queries INVOKER/RLS.
-- Also handle invalid owner filters safely and provide a stable pagination tie-break.
